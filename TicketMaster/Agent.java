...
public class Agent {
    ...
    // - keyword_login - for personal profile page.
    static String keyword_login = "Profile Overview";
    // - keyword_profile - for profile line in profile page.
    static String keyword_profile = "Name:";
    // - keyword_checkout - for checkout page.
    ...
    public static void getOptions(String[] args) throws Exception {
        ...
        // 2 modes: is_opt_name / is_opt_value
        Boolean is_key = true;
        String key = "", arg = "";
        ...
        if (DEBUG < 0 || DEBUG > 7) DEBUG = 0;
    }

    // option value should be non-negative integer.
    // if < 0, then it's error.
    ...
    private static Boolean debugCookie() { return (DEBUG & 1) != 0; }

    // Show get/post details.
    private static Boolean debugGetPost()   { return (DEBUG & 2) != 0; }

    // Displays page html.
    private static Boolean debugHtml() { return (DEBUG & 4) != 0; }
  
    // For this option, in html all "<" is translated to "<br/>&lt;", 
    // so is easy to view in browser.
    private static Boolean debugHtmlSrc() { return (DEBUG & 8) != 0; }
    ...
    // Every config line has the format: key = value
    ...
    // Exception for ticketDate: if empty, get default which is tomorrow.
    ...
    // If there is any error, throw the exception,
    // let getConfig() handle it.
    ...
    // @param: source 1 - direct login, 2 - doCheckTicket.
    ...
    private String sendPost(String url, String postParams) throws Exception {
        ...
        if (debugHtml()) output(page); 
        else if (debugHtmlSrc()) output(page.replace("<", "<br/>&lt;")); 
